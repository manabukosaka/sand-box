...
| 変数名 | 説明 | デフォルト値 |
| --- | --- | --- |
| `DEFAULT_API_KEY` | エージェント等からのデータ送信時に使用されるデフォルトAPIキー | `minidog-test-key` |
...
## 5. 動作確認 (Testing)
`curl` コマンドを使用して、サーバーにログデータを送信し、動作を確認できます。デフォルトの API キー (`minidog-test-key`) を用いてテストを行ってください。

```bash
# ログ送信テスト
curl -X POST http://localhost:3000/api/logs \
  -H "Authorization: Bearer minidog-test-key" \
...
送信後、ブラウザで `http://localhost:3000` のログ検索画面にアクセスし、該当のログが表示されるか確認します。
