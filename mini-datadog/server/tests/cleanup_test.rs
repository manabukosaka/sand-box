...
use mini_datadog_server::db::{init_db, get_oldest_timestamp};
use mini_datadog_server::{run_cleanup_cycle, models::LogRecord};
...
async fn test_cleanup_concurrency() {
    ...
    let cleanup_handle = tokio::spawn(async move {
        run_cleanup_cycle(&db_clone, 30).await
    });
    ...
    let worker_handle = tokio::spawn(async move {
        ...
        for _ in 0..50 { // 50回バッチ処理 (合計5000件)
            for _ in 0..100 {
            ...
            }
            {
                ...
                for r in buffer.drain(..) {
                    appender.append_row(duckdb::params![
                        r.timestamp, r.received_at, r.level, r.service, r.message, None::<String>, None::<String>
                    ]).unwrap();
                }
    ...
    {
        let conn = db.lock().unwrap();
        let mut stmt = conn.prepare("SELECT count(*) FROM logs WHERE service = 'ingest'").unwrap();
        let count: i64 = stmt.query_row([], |r| r.get(0)).unwrap();
        assert_eq!(count, 5000, "All new ingested logs should be present");
        
        let mut stmt = conn.prepare("SELECT count(*) FROM logs WHERE service = 'bench'").unwrap();
...
async fn test_resource_management_checkpoint() {
    ...
    println!("Initial size (after 100k insert + checkpoint): {} bytes", size_initial);
    
    // 2. クリーンアップ実行 (これに CHECKPOINT が含まれている)
    run_cleanup_cycle(&db, 30).await.expect("Cleanup failed");
    
    let size_after_cleanup = fs::metadata(db_path).unwrap().len();
    println!("Size after cleanup/checkpoint: {} bytes", size_after_cleanup);
    ...
    println!("Size after refill + checkpoint: {} bytes", size_after_refill);
    
    // 削除した10万件の空き領域に2万件が収まるはずなので、
    // ファイルサイズは初期サイズ（10万件保持時）を超えないはず。
    assert!(size_after_refill <= size_initial * 11 / 10, "File size should be maintained within reasonable limits by reuse");
